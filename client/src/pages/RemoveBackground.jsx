...
const RemoveBackground = () => {
          ...
          const onSubmitHandler =  async(e) =>{
            {/* prevents the reloding of the page when submitting the form */}
            e.preventDefault();
             try{
                  setLoading(true)
  ...
  return (
     <div className='h-full overflow-y-scroll p-6 flex items-start  gap-4 text-slate-700'>
      {/* left col */}
      <form onSubmit={onSubmitHandler} className='w-full md:w-1/2 max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
       ...
       <p className='text-xs text-gray-500 font-light mt-1'>Supports JPG,PNG and other image formats</p>
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 
         ...
         text-sm rounded-lg cursor-pointer'>
          ...
          {
            loading?<span className='w-4 h-4 my-1 rounded-full border-2 border-t-transport
           animate-spin'></span>
            :
      ...
      {/* right col */}
      <div className='w-full md:w-1/2 max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 
         max-h-[600px]'>
          <div className='flex items-center gap-3'>
